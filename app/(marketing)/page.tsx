    ...
    <div style={{ fontFamily: 'DM Sans, sans-serif', background: '#F5F3EF', color: '#1F2E23' }}>
      ...
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, background: 'rgba(31,46,35,0.97)', borderBottom: '1px solid rgba(184,155,94,0.15)', padding: '0 40px', height: '64px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', gap: isMobile ? '8px' : '28px', alignItems: 'center' }}>
          ...
          <button onClick={() => window.location.href='/sign-in' style={{ padding: '9px 22px', background: 'transparent', color: '#B89B5E', border: '1px solid rgba(184,155,94,0.4)', borderRadius: '4px', fontSize: '12px', letterSpacing: '.1em', textTransform: 'uppercase', cursor: 'pointer', fontWeight: 500 }}>Sign in</button>
          <button onClick={() => window.location.href='/sign-up' style={{ padding: '9px 22px', background: '#B89B5E', color: '#1F2E23', border: 'none', borderRadius: '4px', fontSize: '12px', letterSpacing: '.1em', textTransform: 'uppercase', cursor: 'pointer', fontWeight: 700 }}>Lock in price</button>
        </div>
      </nav>
